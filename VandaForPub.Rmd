...
The fact of the matter is that we sampled different features and in different depths and these will interact, so our stats will have to account for that. As such the question we are really asking is "When controlling for depth, do the different features of a pinnacle contain different microbial communities" and "When controlling for feature, do communities at different depths differ?" 

set working directory, load packages, and point to data
...
alright, so, these aren't great. The quality in the forward reads drops around 150 and the quality of the reverse reads is much better and drops around 225 bp. Usually reverse reads are worse, so this is a bit odd.
```{r}
...
Now we need to learn error rates so that they can be applied to correct the data. This may take awhile. 
```{r}
V2013_errF <- learnErrors(V2013_filtFs, multithread=TRUE)
V2013_errR <- learnErrors(V2013_filtRs, multithread=TRUE)
plotErrors(V2013_errF, nominalQ=TRUE)
```

These look pretty good - the black lines match the error rates much better than the red (default) metrics.

now we apply the learned error rates to our sequences and merge the forward and reverse sequences to figure out how many unique sequences there would be if there were no errors. This may also take awhile. 
...
We have 154 samples with 4087 unique sequences. That seems reasonable. 
...
```
We removed ~155 sequences but only ~0.05% of the reads. 
...
okay, we have 232 bimearas but again they only make up ~0.05% of the community. 

Now we need to classify the sequences. We are including trying the reverse compliment. This is becasue they fail to classify without this and it would make sense for them to be RC because the R reads have quality scores more similar to F reads above. 
```{r}
...
```
okay, now we can do analyses in phyloseq
...
samdf<-read.csv("V2013_Metadata.csv", row.names=1)
...
and those that are not bacteria

```{r}
V2013_large<- prune_samples(sample_sums(ps)>=1000, ps)
V2013_bac<-subset_taxa(V2013_large, Kingdom=="Bacteria")
V2013_bacteria<-subset_taxa(V2013_bac, Order!="Chloroplast")
```
...
We want to compare the subsamples from each depth (e.g. pink 19 to pink 31) and the subsamples within each depth to one another (e.g. pink 19 to brown 19) but we don't want to cross compare (e.g. we don't care about pink 19 comparing to brown 31) so we will do all possible pairwise comparisons using adonis.pair and then we will pull out only those that we care about and use a correction to correct for multiple comparisons. We also don't care at all about webs because there are too few samples to be concerned with. 
...
no_reps_adonis_subsample_depth = adonis.pair(d, no_reps_frame$depth_subsample)
no_reps_adonis_subsample_depth
#and we subsample for only the pairwise comparisons we are interested in
no_reps_adonis_subsample_depth_of_interest = no_reps_adonis_subsample_depth[c(13,14,16,18,20,22,25,27,29,31,33,34,35,37,39,41,44,46,48,50,51,52,54,56,59,61,63,64,65,67,70,72,73,74,77,78),]
...
```
okay, lets see what species are driving the differences between communities that do differ significantly. We can use simper for that. We're really only interested in the cyanobacteria because those are the taxa that we would expect to be different due to the light environment and as the primary producers. We'll parse those out at a later step. 
...
relevant_pink_simper<-subset(pink_simper, pink_simper$average > 0.009999999999 & pink_simper$p < 0.050) #there are none
...
tip_ridge_31_ps = merge_phyloseq(tip_31_ps,ridge_31_ps)
OTUs = as(otu_table(tip_ridge_31_ps), "matrix")
if(taxa_are_rows(tip_ridge_31_ps)){OTUs <- t(OTUs)}
OTUs = as.data.frame(OTUs)
tip_ridge_31_simper <- simper(OTUs, tip_ridge_31_ps@sam_data$subsample, permutations = 100)
tip_ridge_31_simper<-as.data.frame(tip_ridge_31_simper$"2tip_3ridge")
relevant_tip_ridge_31_simper<-subset(tip_ridge_31_simper, tip_ridge_31_simper$average > 0.009999999999 & tip_ridge_31_simper$p < 0.050)
...
simper_all_relevant_ps = prune_taxa(c("ASV1", "ASV2", "ASV3", "ASV4", "ASV5", "ASV6","ASV7","ASV8","ASV10","ASV11", "ASV12",  "ASV14", "ASV15", "ASV17","ASV19","ASV20", "ASV21", "ASV22","ASV27", "ASV38", "ASV40","ASV42", "ASV45","ASV51", "ASV63"), no_reps_ps)
...
simper_relevant_SE<-ddply(simper_relevant_df,c("depth_subsample", "depth", "subsample"),summarise,
                ...
                ASV42=sd(ASV42,na.rm=TRUE)/sqrt(N),
...
#remove web samples because we could not do statistics on them (only two samples, both with 2 replicates each)
simper_relevant_mean<-simper_relevant_mean[c(2:13),]
simper_relevant_SE<-simper_relevant_SE[c(2:13),]
```
...
```{r barplots}
summary_mean_long <-gather(simper_relevant_mean,taxon, mean, ASV1:ASV63 ,factor_key = TRUE)
summary_SE_long <-gather(simper_relevant_SE, taxon, SE, ASV1:ASV63 ,factor_key = TRUE)
...
ggplot(summary_all_long, aes( x=depth, y=mean,fill=subsample,ymin=mean-SE, ymax=mean+SE))  +  
  ...
  scale_fill_manual (values=c("burlywood2", "gray", "chocolate4", "olivedrab", "mediumorchid1", "bisque3"),labels=c("Tip", "Ridge", "Brown", "Green", "Pink", "Beige")) + 
  xlab("Depth") + 
...
ord.nmds.bray_31
```
These are okay values, not great but not horrible either. 
...
dev.off()
```
Calculate the influence of distance into the pinnacle on the location of points in the NMDS plot using vegan's envfit. I will map these onto the NMDS plots manually.
```{r envfit}
